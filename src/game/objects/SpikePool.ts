...
class SpikePool extends GameObject {
    ...
    constructor(sceneKey: string, initsize: number, pos?: IVector2, scale?: IVector2) {
        ...
        this.spikePool = Array.from(
            { length: 8 },
            () => new Spike(sceneKey, new Vector2(-100, -100), new Vector2(50, 50))
        )
        this.transform.rotation = 90
        for (var i = 0; i < initsize; i++) {
            this.spikeList[i] = this.spikePool[i]
        }
        Observer.attach('touch', 'show', () => {
            this.show()
        })
    ...
    public update(delta: number): void {
        ...
        for (var s of this.spikeList) {
            if (
                s.transform.position.x < this.offset[0] ||
                s.transform.position.x > this.offset[1]
            ) {
                s.rb.velocity.x = v
            } else {
                s.rb.velocity.x = 0
